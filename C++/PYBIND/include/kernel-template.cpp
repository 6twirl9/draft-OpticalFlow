...
//, ARGSPEC::VAR_t ("tau","lambda","theta","nscales","warps","epsilon","iterations","scaleStep","gamma","useInitialFlow")
 ...
 auto info_() { return argspec.as_pydict(cur_opt) ; }
...
#define DFLT_CS cv::cuda::Stream& s = cv::cuda::Stream::Null()

 #if __H2D_CONVERSION__
 VarMat   toKerMat(const cv::Mat& in) { KerMat _ ; _.upload(in) ; return _ ; }
 #else
