...
bool load(string impl = "", bool verbose = false)  // ===
{
 bool found = false ;
...
auto operator()(Ts&& ... args)  // ===
{
 ...
 {
 #define rep(r, data, s) rep_(s,BOOST_PP_STRINGIZE(s))
 #define rep_(s,s_str) if( impl == s_str ) { return CALL(get<s>(Ker[s_str]),forward<Ts>(args) ... ) ; }
...
try
{
 collect = IMPL::available("TPU","DualTVL1") ;
}
catch( IMPL::NotImplemented& e)
{
 cout << e.what() << endl ;
